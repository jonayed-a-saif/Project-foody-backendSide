...
export default {
    name : "Edit",
    mounted(){
        axios.get(`/edit/slider/${this.$route.params.sliderId}`)
            .then((response)=>{
    ...
    methods: {
        updateSlider () {
            this.form.post(`/update/slider/${this.$route.params.sliderId}`)
            .then(({ response }) => {
                this.$router.push('/slider-list');
                toast.fire({
                    icon: 'success',
                    title: 'Slider Info Updated'
                })
        ...
        uploadImage(){
            ...
            if(img.length>50){
                ...
                return `/slider_images/${this.form.image}`
            }
        },
    }
}
